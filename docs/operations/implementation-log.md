...
# 2026-09-22 历史原子知识候选收敛

- 原子化准入改为“有效证据、唯一领域、完整陈述”三项同时满足；没有来源证据的结论不再生成 claim。
- 多领域父问题不再按字母排序选取领域。知识点正文具有唯一领域时使用正文领域；正文无领域且父问题不唯一时直接隔离。
- 箭头流程、源码路径、长哈希、本机源码链接、Shell 命令、安装包链接、语言标签、日志残片、单纯大小、测试或构建状态、实施动作及代理执行叙述不再进入原子知识候选。
- 完整性门禁要求句子以句号、问号或感叹号结束；分号列表、未闭合代码标记、内嵌流程箭头和绝对本机路径会被隔离。
- 原始事件和过程知识单元保持不可变；生产只清理并重建 `process_knowledge_claims`、关系和审核派生数据。
- 所有重建结果仍为 `candidate`，只有人工确认的 claim 才能进入公共候选，公共知识继续保持 `shadow`。
- 最终生产重建扫描 6,255 条过程知识，执行 1,997 次候选写入，形成 861 条去重原子知识和 753 条顺序关系，任务失败 0。
- 生产数据门禁结果：缺少证据 0、无效证据关联 0、不完整陈述 0、非法领域 0、DLP 命中 FEC/UDP/`nb_live` 等网络漂移 0。
- 861 条原子知识全部保持 `candidate/unverified`，人工确认 0、公共发布 0；旧公共知识仍为 95 条 `withdrawn`，查询模式保持 `shadow`。
- 最终服务端二进制 SHA-256 为 `921b2a304f8264a2749a8283e4034e39962bf5b8a8bd18d8eae30800bc17971a`，回滚备份位于 `/opt/aetheris/backups/claim-complete-5025b04-20260922-234407`。
