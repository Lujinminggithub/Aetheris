...
export interface Session { user_id: string; username: string; access_role: string; tenant_id?: string; expires_at?: string; permissions?: Record<string, boolean> }
...
export interface RAGFilters { from?: string; to?: string; device_id?: string; project_id?: string; logical_project_id?: string; knowledge_scope?: 'project_process' | 'tenant_and_public'; scope_mode?: 'auto' | 'manual'; all_projects?: boolean; activity_type?: ActivityType }
export interface RAGCitation { number: number; document_id: string; fact_id: string; canonical_event_id: string; source_event_ids: string[]; project_id: string; project_name: string; activity_type: ActivityType | 'process_knowledge' | 'public_knowledge'; excerpt: string; score: number; occurred_at: string; knowledge_id?: string; public_knowledge_id?: string; chunk_id?: string; source_kind?: string; source_scope?: 'tenant_private' | 'platform_public' | 'model_general'; revision?: number; anonymous_source_tenant_count?: number; decision_state?: string; validation_state?: string; applicability?: string; topic?: string }
...
export interface ProcessKnowledgeSummary { sessions: number; units: number; verified: number; conflicts: number; unattributed: number; chunks: number; active_version: number; mode: 'shadow' | 'canary' | 'active' }
export interface ProcessKnowledgeEvidence { evidence_id: string; evidence_kind: string; event_id?: string; fact_id?: string; turn_id?: string; supports_section: string; relation: string; reason_code: string }
export interface ProcessKnowledgeUnit { knowledge_id: string; session_id: string; logical_project_id: string; topic: string; knowledge_type: string; problem: string; intent: string; constraints: string; conclusion: string; rationale: string; alternatives: string; applicability: string; caveats: string; decision_state: 'proposed' | 'accepted' | 'rejected' | 'superseded'; validation_state: 'unverified' | 'partially_verified' | 'verified' | 'contradicted'; lifecycle_state: 'candidate' | 'active' | 'stale' | 'withdrawn'; occurred_at: string; revision: number; version: number; evidence: ProcessKnowledgeEvidence[] }
export interface ProcessKnowledgePage { units: ProcessKnowledgeUnit[]; total: number; limit: number; offset: number }
export interface ProcessKnowledgeClaim { claim_id: string; knowledge_id: string; revision: number; sequence: number; domain: string; entities: string[]; problem: string; claim: string; applicability: string; validation_state: 'unverified' | 'partially_verified' | 'verified' | 'contradicted'; lifecycle_state: 'candidate' | 'confirmed' | 'rejected' | 'superseded'; evidence_ids: string[]; created_at: string; updated_at: string }
export interface ProcessKnowledgeClaimPage { claims: ProcessKnowledgeClaim[]; total: number; limit: number; offset: number }
...
export interface PublicKnowledgeRevision { revision: number; problem_pattern: string; conclusion: string; rationale: string; applicability: string; caveats: string; alternatives: string; validation_state: PublicKnowledgeValidationState; anonymous_source_tenant_count: number; independent_session_count: number; canonical_hash: string; extractor_version: string; redaction_version: string; review_policy_version: string; created_at: string }
...
export interface PublicKnowledgeUnit { public_knowledge_id: string; canonical_topic: string; knowledge_type: string; publication_state: PublicKnowledgePublicationState; current_revision: number; domains: string[]; entities: string[]; scope_state: 'classified' | 'unclassified' | 'rejected'; current: PublicKnowledgeRevision; reviews?: PublicKnowledgeReview[]; private_evidence?: PublicKnowledgePrivateEvidence[]; created_at: string; updated_at: string }
export interface PublicKnowledgePage { units: PublicKnowledgeUnit[]; total: number; limit: number; offset: number }
