...
import type { ActivitiesResult, ActivityType, EventDetail, RAGQueryJob, RAGStatus } from '../api/types'
...
export default function RAGQueryPage() {
  ...
  const [question, setQuestion] = useState(''); const [status, setStatus] = useState<RAGStatus | null>(null); const [facets, setFacets] = useState<ActivitiesResult | null>(null)
  ...
  useEffect(() => { let active = true; Promise.all([api.getRAGStatus(), api.listActivities({ from, to, limit: 1, offset: 0 })]).then(([nextStatus, nextFacets]) => { if (active) { setStatus(nextStatus); setFacets(nextFacets) } }).catch(reason => { if (active) setError(reason instanceof Error ? reason.message : '检索状态加载失败') }); return () => { active = false } }, [from, to])
  ...
  async function submit() {
    ...
    try {
      const created = await api.createRAGQuery({ question: question.trim(), filters: { from, to, device_id: deviceID || undefined, project_id: projectID || undefined, activity_type: activityType || undefined } })
      setJob({ query_id: created.query_id, question, filters: { from, to }, status: created.status as RAGQueryJob['status'], progress: created.progress, answer: '', citations: [], created_at: new Date().toISOString() })
  ...
  return <section>
    ...
    <div className="rag-scope">
      ...
      <label>项目范围<select aria-label="项目范围" value={projectID} onChange={event => setProjectID(event.target.value)}><option value="">全部项目</option>{facets?.projects.map(item => <option value={item.id} key={item.id}>{item.label}</option>)}</select></label>
