...
describe('智能查询页面', () => {
  ...
  it('异步查询并通过引用打开活动证据', async () => {
    ...
    expect(api.createRAGQuery).toHaveBeenCalledWith({ question: '最近修复了什么？', knowledge_scope: 'tenant_and_public' })
  ...
  it('默认只显示简洁答案并折叠技术证据', async () => {
    ...
    vi.spyOn(api, 'listProjects').mockResolvedValue({ projects: [], count: 0 })
  ...
  it('默认自动范围不要求日期设备或项目', async () => {
    ...
    expect(screen.queryByLabelText('设备范围')).not.toBeInTheDocument()
    expect(screen.getByRole('button', { name: '开始查询' })).toBeEnabled()
    await userEvent.click(screen.getByRole('button', { name: '开始查询' }))
    expect(api.createRAGQuery).toHaveBeenCalledWith({ question: '分析 Windows EDR', knowledge_scope: 'tenant_and_public' })
  })
