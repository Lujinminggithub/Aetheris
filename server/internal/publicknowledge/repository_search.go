...
func (repository *Repository) LoadPublicVectorCandidates(ctx context.Context, hits []retrieval.Hit) ([]SearchCandidate, error) {
	...
	rows, err := repository.pool.Query(ctx, `SELECT c.chunk_id,c.public_knowledge_id,c.canonical_topic,c.knowledge_type,c.validation_state,c.content,r.applicability,c.revision,r.anonymous_source_tenant_count
        ...
        WHERE c.chunk_id=ANY($1) AND c.index_status='indexed' AND u.publication_state='published' AND r.validation_state='platform_certified'`, ids)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err = rows.Scan(&item.ChunkID, &item.PublicKnowledgeID, &item.CanonicalTopic, &item.KnowledgeType, &item.ValidationState, &item.Content, &item.Applicability, &item.Revision, &item.AnonymousSourceTenantCount); err != nil {
			return nil, err
		}
...
func (repository *Repository) PublicKeywordCandidates(ctx context.Context, terms []string, limit int) ([]SearchCandidate, error) {
	...
	rows, err := repository.pool.Query(ctx, `SELECT c.chunk_id,c.public_knowledge_id,c.canonical_topic,c.knowledge_type,c.validation_state,c.content,r.applicability,c.revision,r.anonymous_source_tenant_count,
        ...
        JOIN public_knowledge_units u ON u.public_knowledge_id=c.public_knowledge_id AND u.current_revision=c.revision
        JOIN public_knowledge_revisions r ON r.public_knowledge_id=c.public_knowledge_id AND r.revision=c.revision
        WHERE c.index_status='indexed' AND u.publication_state='published' AND r.validation_state='platform_certified'
	...
	for rows.Next() {
		var item SearchCandidate
		var exactHits int
		if err = rows.Scan(&item.ChunkID, &item.PublicKnowledgeID, &item.CanonicalTopic, &item.KnowledgeType, &item.ValidationState, &item.Content, &item.Applicability, &item.Revision, &item.AnonymousSourceTenantCount, &item.Score, &exactHits); err != nil {
			return nil, err
		}
