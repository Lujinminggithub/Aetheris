...
func (generator *ModelGatewayGenerator) GenerateAnswer(ctx context.Context, question string, mode AnswerMode, citations []Citation, correction string) (GeneratedAnswer, error) {
	...
	for index, citation := range modelCitations {
		evidence[index] = map[string]any{"number": citation.Number, "knowledge_id": citation.KnowledgeID, "source_kind": citation.SourceKind, "validation_state": citation.ValidationState, "applicability": citation.Applicability, "content": truncate(citation.Excerpt, 150)}
...
func buildAnswerPlan(question string, mode AnswerMode, citations []Citation) AnswerPlan {
	...
	for _, citation := range citations {
		...
		claim := strings.TrimSpace(citation.Topic)
		if claim == "" {
			claim = "本地过程结论"
		}
		claims = append(claims, AnswerPlanClaim{Claim: truncate(claim, 30), SourceKind: citation.SourceKind, KnowledgeUnitIDs: []string{citation.KnowledgeID}, Applicability: truncate(citation.Applicability, 30)})
